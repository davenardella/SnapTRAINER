...
begin
  if Online then
     ...
     for c := 0 to ItemsCount-1 do
     begin
       Items^[c].Result:=0;
       FillChar(Items^[c].pdata^,Items^[c].Amount,#0);
     end;
     Result:=0;
  end;
end;
...
begin
  if Par_GetStatus(HP, Result{%H-})<>0 then
    Result:=errLibInvalidObject;
end;
//------------------------------------------------------------------------------
...
begin
  DW[0]:=pbyte(NativeUInt(pval)+3)^;
  DW[1]:=pbyte(NativeUInt(pval)+2)^;
  DW[2]:=pbyte(NativeUInt(pval)+1)^;
  DW[3]:=pbyte(pval)^;
end;
//------------------------------------------------------------------------------
...
begin
  pbyte(pval)^:=DW[3];
  pbyte(NativeUInt(pval)+1)^:=DW[2];
  pbyte(NativeUInt(pval)+2)^:=DW[1];
  pbyte(NativeUInt(pval)+3)^:=DW[0];
end;
//------------------------------------------------------------------------------
...
begin
  pbyte(pval)^:=DW[3];
  {%H-}pbyte(NativeUInt(pval)+1)^:=DW[2];
  pbyte({%H-}NativeUInt(pval)+2)^:=DW[1];
  pbyte({%H-}NativeUInt(pval)+3)^:=DW[0];
end;
//------------------------------------------------------------------------------
